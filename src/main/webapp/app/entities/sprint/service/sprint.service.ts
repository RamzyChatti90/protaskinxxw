...
type RestOf<T extends ISprint | NewSprint> = Omit<T, 'startDate' | 'createdAt'> & {
...
export class SprintService {
  ...
  protected convertDateFromClient<T extends ISprint | NewSprint | PartialUpdateSprint>(sprint: T): RestOf<T> {
    return {
      ...
      createdAt: sprint.createdAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSprint: RestSprint): ISprint {
    return {
      ...
      createdAt: restSprint.createdAt ? dayjs(restSprint.createdAt) : undefined,
    };
  }
